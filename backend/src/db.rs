    ...
    vec![
        ...
        format!(
            "ALTER TABLE {schema}.positions
             ADD COLUMN IF NOT EXISTS snapshot_id BIGINT GENERATED BY DEFAULT AS IDENTITY;",
            schema = schema,
        ),
        format!(
            "DO $$ DECLARE seq_name text;
                BEGIN
                    SELECT pg_get_serial_sequence('{schema}.positions', 'snapshot_id') INTO seq_name;
                    IF seq_name IS NOT NULL THEN
                        EXECUTE format('UPDATE {schema}.positions SET snapshot_id = COALESCE(snapshot_id, nextval(''%s''::regclass)) WHERE snapshot_id IS NULL', seq_name);
                    END IF;
                END $$;",
            schema = schema,
        ),
        ...
        format!("DROP INDEX IF EXISTS {schema}_positions_inst_id_pos_side_uindex;", schema = schema),
        format!(
            "CREATE UNIQUE INDEX IF NOT EXISTS {schema}_positions_inst_id_pos_side_open_idx ON {schema}.positions (inst_id, pos_side) WHERE closed_at IS NULL;",
            schema = schema,
        ),
        format!(
            "CREATE UNIQUE INDEX IF NOT EXISTS {schema}_positions_snapshot_id_uindex ON {schema}.positions (snapshot_id);",
            schema = schema,
        ),
        format!(
            "DO $$ BEGIN
                ALTER TABLE {schema}.positions
                ADD CONSTRAINT {schema}_positions_entry_ord_fk
                    FOREIGN KEY (entry_ord_id) REFERENCES {schema}.orders (ord_id) ON DELETE SET NULL;
            EXCEPTION
        ...
        format!(
            "DO $$ BEGIN
                ALTER TABLE {schema}.positions
                ADD CONSTRAINT {schema}_positions_exit_ord_fk
                    FOREIGN KEY (exit_ord_id) REFERENCES {schema}.orders (ord_id) ON DELETE SET NULL;
            EXCEPTION
                WHEN duplicate_object THEN NULL;
            END $$;",
            schema = schema,
...
pub async fn fetch_position_snapshots(
    ...
    let mut sql = format!(
        "SELECT
            ...
            mark_px::double precision AS mark_px,
            margin::double precision AS margin,
...
pub async fn upsert_position_snapshot(snapshot: PositionSnapshot) -> Result<()> {
    ...
    let sql = format!(
        ...
        ON CONFLICT (inst_id, pos_side) WHERE (positions.closed_at IS NULL) DO UPDATE SET
