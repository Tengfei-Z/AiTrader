...
- 严禁同一合约多空同持，反手必须先平掉原方向并再次确认仓位清零。
...
5. 严禁同一合约同时持有多头与空头对冲仓位；若需要反手，必须先依据最新 `get_positions` 结果，用相反 `posSide` 的 `place_order` 彻底平掉原方向，并再次调用 `get_positions` 确认仓位已清零后，才能建立新的方向性仓位。
6. 所有操作都需有对应理由并输出在响应中。"""
